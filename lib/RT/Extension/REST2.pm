...
You can also provide an initial content message in your ticket by adding a C<Content> property to the JSON object. The value of this C<Content> property is a plain text string by default. You can also use a C<HTML> string, in which case you have also to add a C<ContentType> set to C<text/html> to the JSON object.

Finally, you can attach any binary or text file to your ticket by specifying in the JSON object sent a C<AttachementsContents> property, which should be a JSON array where each item represents a file you want to attach. Each item is a JSON object with the following properties:
...
And here is a full example to create a ticket with an image attached. Since the C<MIME Base64> string encoding the image file might be too large for the shell's command line, we echo the JSON object and pipe the whole string to curl (wrapped to multiple lines for readability):
